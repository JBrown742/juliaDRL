 mutable struct DQN{S, A} <: LearningAlgorithm
    ...
    n::Int
    """
        DQN(model::LearningModel, lr::Float32, reward_func::Function, batch_size::Int, γ::Float32, ϵ::Float32, optimizer::Flux.Optimise.Optimiser, max_buffer_len::Int)

    Struct for dealing with DQN problems.
    """
    function DQN(::Type{S},::Type{A}, model::LearningModel, policy::AbstractPolicy, 
        batch_size::Int, γ::Float64, ϵ::Float64, τ::Float32, optimizer::O) where {S,A,O <: Flux.Optimise.AbstractOptimiser}
        replay = Buffer{S,A}(50000)
        return new{S,A}(replay, model, deepcopy(model), policy, batch_size, γ, ϵ, τ, optimizer, 1)
    end
    function DQN(::Type{S},::Type{A}, model::LearningModel, policy::AbstractPolicy, 
        batch_size::Int, γ::Float64, ϵ::Float64, τ::Float32, optimizer::O, n::Int) where {S,A,O <: Flux.Optimise.AbstractOptimiser}
        replay = Buffer{S,A}(n)
        return new{S,A}(replay, model, deepcopy(model), policy, batch_size, γ, ϵ, τ, optimizer, n)
    end
end

## The environment is still baked into this algo...
function train!(alg::DQN{S,A}) where {S,A}
    θ = Flux.params(alg.model) # get the model parameters. # Model needs to be in a consistent format whereby θ encapsulates all params
    ...
    # get return 
    next_state_outputs = alg.model(next_states)
    ...
    model_outputs_untracked = alg.model(states) 
    ...
    ∇ = Flux.gradient(θ) do # track gradients
        model_outputs = alg.model(states) 
...
function learning_episode!(env::E, alg::DQN{S,A}) where {E <: AbstractEnv, S,A}
    ...
    while env.terminal == false # bool flag to denote whether routing has finished
        # Generalises below...
        action = alg.policy(env, alg; ϵ=alg.ϵ)
